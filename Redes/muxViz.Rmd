...
############
```{r}
SupraAdjacencyToBlockTensor(SupraAdjacencyMatrix = sam, Layers = layers,
                            Nodes = nodes)
```
...
GetMultiHubCentrality(SupraAdjacencyMatrix = sam, Layers = layers,
                      Nodes = nodes)
```
Toma en cuenta el degree de los nodos de las capas, pero no las conexiones entre capas. Cuenta conexiones de entrada y salida, en redes no dirigidas son equivalentes.
```{r}
GetMultiInDegree(SupraAdjacencyMatrix = sam, Layers = layers, Nodes = nodes,
                 isDirected = F)
GetMultiOutDegree(SupraAdjacencyMatrix = sam, Layers = layers, Nodes =nodes,
                  isDirected = F)
```
Este incluye también las conexiones entre capas.
```{r}
GetMultiInDegreeSum(SupraAdjacencyMatrix =sam, Layers = layers, Nodes = nodes,
                    isDirected = F)
GetMultiOutDegreeSum(SupraAdjacencyMatrix = sam, Layers = layers, Nodes = nodes,
                     isDirected = F)
```
...
GetMultiStrength(SupraAdjacencyMatrix = sam, Layers = layers, Nodes = nodes,
                 isDirected = F)
#Concidera también las conexiones entre capas
GetMultiStrengthSum(SupraAdjacencyMatrix = sam, Layers = layers, Nodes = nodes,
                    isDirected = F)
#Conexiones de entrada
GetMultiInStrength(SupraAdjacencyMatrix = sam, Layers = layers, Nodes = nodes,
                   isDirected = F)
#Conexiones de salida
GetMultiOutStrength(SupraAdjacencyMatrix = sam, Layers = layers, Nodes = nodes,
                    isDirected = F)
```
En este caso, además del número de conexiones con sus respectivos pesos, se incluyen también las conexiones entre capas y sus pesos. Tanto este como el caso anterior son prácticos para redes pesadas. El primero no toma en cuenta las conexiones entre capas.
```{r}
GetMultiStrength(SupraAdjacencyMatrix = sam, Layers = layers, Nodes =nodes,
                 isDirected = F)
GetMultiInStrengthSum(SupraAdjacencyMatrix = sam, Layers = layers,
                      Nodes = nodes, isDirected = F)
GetMultiOutStrengthSum(SupraAdjacencyMatrix = sam, Layers =layers,
                       Nodes = nodes, isDirected = F)
```
...
GetMultiKCoreCentrality(SupraAdjacencyMatrix = sam, Layers = layers,
                        Nodes = nodes)
```
Mide el grado relativo de influencia de un nodo en una red tomando en cuenta solo las rutas geodésicas, es decir, las distancias más cortas entre un nodo con respecto a otros. Penaliza las conexiones con los nodos más distantes mientras que da prioridad a los vecinos inmediatos.
```{r}
GetMultiKatzCentrality(SupraAdjacencyMatrix = sam, Layers = layers,
                       Nodes = nodes)
```
Es un ajuste a la centralidad de Katz basado en un modelo Random-Walk. Toma en cuenta el número de enlaces que recibe un nodo, naturalmente entre mayor sea este número, mayor la importancia del nodo. Este algoritmo prioriza los enlaces con nodos de mayor jerarquía, es decir, con nodos que a su vez podean mayor centralidad, además de premiar los enlaces parcimoniosos.
```{r}
GetMultiPageRankCentrality(SupraAdjacencyMatrix = sam, Layers = layers,
                           Nodes = nodes)
```

Genera una matriz de distancias, además de calcular la media del número de pasos de las rutas más cortas entre todos los pares de nodos de la red (una medida de la eficiencia de la conectividad de la red), así como el coeficiente de cercanía (el núpero  de nodos de la red sobre la suma de las distancias más cortas entre un nodo con el resto de nodos de la red, o simplemente el inverso de dicha sumatoria).
```{r}
GetMultiPathStatistics(SupraAdjacencyMatrix = sam, Layers = layers,
                       Nodes = nodes)
```
En este caso no se promedian las distancias más cortas, sino que el algoritmo promedia la sumatoria de las distancias entre un nodo y sus vecinos calculadas a partir de un algoritmo estocástico de random-walk.
```{r}
GetMultiRWCentrality(
  ...
  Nodes = nodes,
  Type = "classical",
  #"clasical", "pagerank", "diffusive"
  Method = "multilayer"
  #"multilayer", "perlayer"
)
```
Promedia la cantidad de conexiones que un nodo tiene con las demás capas.
```{r}
GetMultiplexityCentrality(SupraAdjacencyMatrix = sam, Layers = layers,
                          Nodes = nodes)
```

```{r}
GetConnectedComponents(SupraAdjacencyMatrix = sam, Layers = layers,
                       Nodes = nodes)
```
```{r}
