...
v_colored<-function(g, g_tax, p_tax, g_colors){
  ...
  g_plot<-plot(g, vertex.color=vertex.attributes(plant_g)$color)
  return(g_plot)
}
```
...
```{r}
unq<-unique(T_table[,"Class"])
colors<-rainbow(length(unq))
v_colored(insect_g, g_tax = "Class", p_tax = "Family", g_colors = colors)
v_colored(plant_g, g_tax = "Class", p_tax = "Family", g_colors = colors)
```
