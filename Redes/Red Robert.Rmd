...
plant_g<-v_colored(plant_g, T_table, g_tax = "Phylum", p_tax = "Genus",
                   g_colors = colors)
plot(insect_g, vertex.label.color="black",
     vertex.color = vertex.attributes(insect_g)$color, vertex.label.cex=.5,
...
#       bty="n", ncol=1)
plot(plant_g, vertex.label.color="black",
     vertex.color = vertex.attributes(plant_g)$color, vertex.label.cex=.5,
     vertex.label.dist=1,layout=layout_with_kk, vertex.size = 5)
...
by_degree<-which(vertex.attributes(d[[1]])$hl == Redpal[1])
vertex.attributes(insect_g)$Group[by_degree]
by_betweenness<-which(vertex.attributes(bt[[1]])$hl == Redpal[1])
vertex.attributes(insect_g)$Group[by_betweenness]
by_closeness<-which(vertex.attributes(cl[[1]])$hl == Redpal[1])
vertex.attributes(insect_g)$Group[by_closeness]
```
...
#Se genera un data.frame que contiene la suma de las abundancias de los phyla
...
```
Posteriormente introducimos la tabla de abundancias totales (T_Collapsed), el número
de nodos que queremos considerar, y las listas de las matrices y las capas de la red,
además del valor de alpha para la prueba de t. Debido a que la función trabaja con
redes bipartitas, las listas solo deben contener dos elementos cada una. La red
resultante es una fusión de las capas de la red bipartita.
```{r}
mat_list<-list(Insectmat, Plantmat)
g.list<-list(insect_g, plant_g)
#Consideramos solo los 20 nodos más importantes y una alpha de 0.1.
diff_g<-diff_nodes_graph(T_Collapsed, n = 20, mat_list, g.list, alpha = 0.1)
